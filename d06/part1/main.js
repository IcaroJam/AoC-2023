...
fs.readFile("../input.txt", "ascii", (err, data) => {
	...
	let ways;
	let mult = 1;
	for (let i = 0; i < raceTable.times.length; i++) {
		ways = getWays(raceTable.times[i], raceTable.distances[i]);
		console.log(`Race ${i + 1}, ${ways} ways.`);
		mult *= ways;
	}

	console.log("Margin of error:", mult);
});
